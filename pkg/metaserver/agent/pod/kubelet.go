...
func (k *kubeletPodFetcherImpl) GetPodList(ctx context.Context, podFilter func(*v1.Pod) bool) ([]*v1.Pod, error) {
	...
	if k.conf.EnableKubeletSecurePort {
		if err := process.GetAndUnmarshalForHttps(ctx, k.conf.KubeletSecurePort, k.conf.NodeAddress, k.conf.KubeletPodsEndpoint, k.conf.APIAuthTokenFile, &podList); err != nil {
